...
describe('Mock API Unit Tests', function () {
  ...
  it("GET /{visa_type} 200", () => {
    return api.request({method: 'GET', path: '/visa_type', query: { mock: '' }})
      .then(res => { ut.testMockResponseSuccess(res, 200); });
  });
  it("PUT /{visa_type}  200", () => {
    return api.request({ method: 'PUT', path: '/visa_type', query: { mock: '' },
      body: {
        "visa_type": "visa_type",
        "description": "visa_type",
        "long_description": "visa_type",
  ...
  it("DELETE /{visa_type} 204", () => {
    return api.request({ method: 'DELETE', path: '/visa_type', query: { mock: '' } })
      .then(res => { ut.testMockResponseSuccess(res, 204); });
  });

  // ----- Country Code Logs -----
  it("GET /{visa_type}/logs 200", () => {
    return api.request({ method: 'GET', path: '/visa_type/logs', query: { mock: '' } })
      .then(res => { ut.testMockResponseSuccess(res, 200); });
  });
